    ...
    class OpenStack
      class Real
        def update_lbaas_l7policy(l7policy_id, options = {})
          ...
          vanilla_options = [:action, :name, :description, :redirect_pool, :redirect_url, :position]
          ...
          request(
            ...
            :expects => [200],
            :method  => 'PUT',
            :path    => "lbaas/l7policies/#{l7_policy_id}"
          )
        end
      end
      ...
      class Mock
        def update_lbaas_l7rule(l7policy_id, options = {})
          ...
          if l7policy = list_l7policies.body['l7policies'].find { |_| _['id'] == l7policy_id }
            ...
            l7policy['redirect_pool'] = options[:redirect_pool]
